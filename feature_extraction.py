...
def to_csv(week, mode, data, ul, uf_dict, list_uf, subsession_mode={}):
    ...
    towrite.to_pickle("res/" + str(week) + mode + ".pickle")
    ...
    if mode == 'session' and len(subsession_mode) > 0:
        for k1 in subsession_mode:
            for k2 in subsession_mode[k1]:
                ...
                df_tmp.to_pickle("res/" + str(week) + mode + k1 + str(k2) + ".pickle")
...
if __name__ == "__main__":
    ...
    [os.mkdir(x) for x in ["res", "ExtractedData", "DataByWeek", "NumDataByWeek"]]
    ...
    for mode in ['week', 'day', 'session']:
        ...
        towrite = pd.read_pickle("res/" + str(weekRange[0]) + mode + ".pickle")
        towrite.to_csv(all_csv, header=True, index=False)
        for w in weekRange[1:]:
            towrite = pd.read_pickle("res/" + str(w) + mode + ".pickle")
        ...
        if mode == 'session' and len(subsession_mode) > 0:
            for k1 in subsession_mode:
                for k2 in subsession_mode[k1]:
                    ...
                    towrite = pd.read_pickle('res/' + str(weekRange[0]) + mode + k1 + str(k2) + ".pickle")
                    towrite.to_csv(all_csv, header=True, index=False)
                    for w in weekRange[1:]:
                        towrite = pd.read_pickle('res/' + str(w) + mode + k1 + str(k2) + ".pickle")
                        towrite.to_csv(all_csv, header=False, index=False)

        print(f'Extracted {mode} data. Time (mins): {(time.time() - st) / 60:.2f}')
        st = time.time()

    [os.system(f"rm -r {x}") for x in ["res", "DataByWeek", "NumDataByWeek"]]

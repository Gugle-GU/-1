...
def split_data(data, test_size=0.25, random_state=0, y_column='insider',
    ...
    insiders_users = set(data[data['insider'] != 0]['user'])
    print("all threaten users ",len(insiders_users))
    ...
    # split data by user
        ...
        print("threaten users ",len(train_users)+len(test_users))
    ...
    # split by user and time
        ...
        first_part_split = split_data(first_part, random_state=random_state, test_size=0.5,
        ...
        print("real threaten users ", len(real_train_insiders))
...
if __name__ == '__main__':
    ...
    # 定义四个不同的分类器
    #byweek LGBMClassifier(random_state=42,n_jobs=n_cores,num_leaves=24,min_child_samples= 24,n_estimators=402,learning_rate=0.05,reg_alpha= 0.5, reg_lambda=0.5)
    #bysession LGBMClassifier(random_state=42,n_jobs=n_cores,num_leaves=21,min_child_samples= 35,n_estimators=282,learning_rate=0.05,reg_alpha= 0.1, reg_lambda=0.5)
    clfs = {'LightGBM': LGBMClassifier(random_state=42,n_jobs=n_cores,num_leaves=24,min_child_samples= 24,n_estimators=402,learning_rate=0.05,reg_alpha= 0.5, reg_lambda=0.5),
    ...
    y_test = res['y_test']
    print("train_users ",len(res['train_users']))
    print("test_users ", len(res['test_users']))
    ...
    for clf_name, clf in clfs.items():
        ...
        # print(cm)
        # # 获取TN，FP，FN和TP的值
        # tn = sum(cm[i][j] for i in range(len(labels)) for j in range(len(labels)) if i != 0 and j != 0)
        # fp = sum(cm[i][j] for i in range(len(labels)) for j in range(len(labels)) if i == 0 and j != 0)
        # fn = sum(cm[i][j] for i in range(len(labels)) for j in range(len(labels)) if i != 0 and j == 0)
        # tp = sum(cm[i][j] for i in range(len(labels)) for j in range(len(labels)) if i == 0 and j == 0)
        #
        # fpr = fp / (fp + tn)
        # pr = tp / (tp + fp)
